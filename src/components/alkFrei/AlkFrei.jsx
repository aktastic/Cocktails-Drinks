...
useEffect(() => {
    ...
    .catch(error => console.error('Error fetching non-alcoholic cocktails:', error));
}, []);
...
useEffect(() => {
    ...
    if (modalEl) {
    modalEl.style.top = `${Math.max(window.innerHeight / 2 - modalEl.offsetHeight / 2, 0)}px`;
    modalEl.style.left = `${Math.max(window.innerWidth / 2 - modalEl.offsetWidth / 2, 0)}px`;
    }
}, [modalOpen]);
...
return (
    ...
    <div className="non-cocktail-grid">
        ...
        <div key={cocktail.idDrink} className="non-cocktail-item" onClick={() => handleCocktailClick(cocktail)}>
            <img src={cocktail.strDrinkThumb} alt={cocktail.strDrink} />
            <p className='non-cocktailName'>{cocktail.strDrink}</p>
        </div>
        ))}
    </div>
    ...
    <Modal open={modalOpen} onClose={handleCloseModal} >
        {selectedCocktail && (
        <div className="non-selected-cocktail">
